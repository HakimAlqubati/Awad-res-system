...
class UserController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $userDataObject->owner_id = $userData->owner_id;
        if ($userData->owner_id) {
            $userDataObject->owner_name = User::where('id', $userData->owner_id)->get()->first()->name;
        } else {
            $userDataObject->owner_name = null;
        }
    ...
    {
        $data = User::whereIn('role_id', array(3))->get();
        return $data;
    }
    /**
    ...
    public function store(Request $request)
    {
        ...
        $validateUser = User::where('email', $request->email)->get()->first();
        $redirect = redirect()->back();
        if ($validateUser) {
            return $redirect->with([
                'message'    => "Duplicate User with email " .  $request->email,
                'alert-type' => 'error',
            ]);
